...
class Spot:
    def __init__(self, spot_id: int, adjacent_hex_ids: list, position: tuple = None, has_port: bool = False):
        ...
        self.player = None                        # Which player (if any) occupies this spot
        self.settlement_type = SettlementType.NONE
        self.has_port = has_port

    def build_settlement(self, player, settlement_type: SettlementType = SettlementType.SETTLEMENT):
        self.player = player
        self.settlement_type = settlement_type

    def __repr__(self):
        return (f"Spot(id={self.spot_id}, hexes={self.adjacent_hex_ids}, position={self.position}, "
                f"player={self.player}, type={self.settlement_type.name}, port={self.has_port})")
