...
class MCTS:
    ...
    """
    def __init__(self, network, state_encoder, action_mapper, num_simulations=800, c_puct=1.5, batch_size=8, noise_eps=0.25, noise_alpha=0.3):
        """
        Initialize the MCTS
    ...
    def search(self, game_state):
        ...
        try:
            ...
            if self.noise_eps > 0:
              # Extract the raw prior probabilities into an array
              priors = np.array([p for (_, p) in action_priors], dtype=np.float64)
              # Sample a Dirichlet noise vector of the same length
              noise  = np.random.dirichlet([self.noise_alpha] * len(priors))
              # Mix them: (1 - ε) * prior + ε * noise
              priors = (1 - self.noise_eps) * priors + self.noise_eps * noise
              # Rebuild action_priors with the noised values
              action_priors = [
                  (action, float(priors[i]))
                  for i, (action, _) in enumerate(action_priors)
              ]
