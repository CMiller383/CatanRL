...
def play_one_game_entry(args):
    ...
    if checkpoint_path and os.path.exists(checkpoint_path):
        ...
        # Load placement network if enabled
    placement_network = None
    if config.get('use_placement_network', False):
        placement_path = os.path.join(config.get('model_dir', 'models'), 'placement_network.pt')
        if os.path.exists(placement_path):
            try:
                from AlphaZero.model.initial_placement_network import InitialPlacementNetwork
                checkpoint = torch.load(placement_path, map_location='cpu')
                if 'network_state_dict' in checkpoint:
                    # Create placement network
                    ...
                    placement_network = InitialPlacementNetwork(
                        input_dim=input_dim,
                        hidden_dim=hidden_dim,
                        output_dim=54
                    )
                    placement_network.load_state_dict(state_dict)
                    placement_network.eval()
                    
                    if game_idx == 0:
                        print(f"[Worker {game_idx}] Loaded placement network")
            except Exception as e:
                print(f"[Worker {game_idx}] ✗ failed to load placement network: {e}")
                placement_network = None
